...
con = lite.connect('../soundscape_affect_experiment.db')

# audio file regions filtered and trimmed to this duration
desiredDuration = 4.0 
corpusName = 'SoundIdeasCorpus'
searchDirectory = '/Volumes/AUME/AUME/Sound_Files/XSeries'

# init the segmenter and train with BF Corpus
segmenter = Segmenter('./feature_output.txt')
output_folder = "../"+corpusName+"/"

# method that segments file
def process_file(file, id):
    rli = segmenter.regions(file) # segment the file
    ...
    if len(candidates):	# if there is more than none
        ...
        if good:
            ...
            filename = os.path.basename(file)
            feats, type = segmenter.features_only(output_folder+"r_"+filename)
            ...
            with con:
                try:
                    cur.execute("INSERT INTO "+corpusName+" (FsID, FileName, Duration, Class, Loud_Mean, Loud_Std,MFCC1_Mean, MFCC1_Std, MFCC2_Mean, MFCC2_Std, MFCC3_Mean, MFCC3_Std) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (id, "r_"+filename, desiredDuration, region[0], feats[0], feats[1], feats[2], feats[3], feats[4], feats[5], feats[6], feats[7]))
                except:
                    print 'not inserted'
...
with con:
    cur = con.cursor()    
    try:
        cur.execute("CREATE TABLE "+corpusName+" (Id INTEGER PRIMARY KEY, FsID INTEGER, FileName TEXT, Duration FLOAT, Class TEXT, Loud_Mean FLOAT, Loud_Std FLOAT, MFCC1_Mean FLOAT, MFCC1_Std FLOAT, MFCC2_Mean FLOAT, MFCC2_Std FLOAT, MFCC3_Mean FLOAT, MFCC3_Std FLOAT)")
    except:
